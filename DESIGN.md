...
区分カードは大画面で2列、760px以下で1列。「修得済み」「必要単位」「予定」「数値上の残り」の順に読む。必要単位を原本に対応付けられない区分には進捗率を表示しない。原本の大区分・科目群・科目の階層を保ち、条件と出典、科目一覧は開閉できるようにする。

検索・履修状況の絞り込みでは区分全体の進捗は維持する。一覧内の科目数と合計単位は表示対象に揃える。状態は色だけでなく文字でも示す。科目ごとに原本ページへ移動できるようにする。

教員・教場の候補には開講年度と受講対象を表示し、クラスを選んだ結果を明示する。「クラス選択」と「時間割に保存」は別操作として保存前の確認を促す。

## 2026-09-07：ブルーを基調にした日常の履修管理

都市大のTCUブルーを参考に、アプリの主色を #0069a8、濃色を #004c7c、背景を #edf6fd とする。公式の指定RGB値を確認できたという主張ではなく、可読性を考えた本アプリの配色。大学サイトの[TCUブルーへの言及](https://www.comm.tcu.ac.jp/cisj/23/assets/23_topics.pdf)を参照した。

ホームは「時間割を開く」を主操作にし、曜日別コマ数を棒グラフで表示する。装飾用の架空の時間割を実データの表示に置き換える。成績画面は累計の数値と表示学年の科目数を区別し、成績別ドーナツ・凡例・検索・未入力フィルタから記録へ進める。

グラフは0始点、立体表現なし。ドーナツの分母は表示学年の登録科目数であり、卒業必要単位ではない。0件でも除算エラーを起こさず、数値と凡例を残す。モバイルではドーナツと凡例を縦積みにする。操作は44px以上、フォーカス表示、動きの軽減設定を尊重する。
