// Build-time migration only. Never imported by the frontend.
import { parseCreditRequirementsFile, parseCoursesFile, parseClassScheduleFile, parseApplicableCoursesFile, parseCreditRequirements, parseCourses, parseClassScheduleRows, mergeCoursesWithScheduleDetailed } from '../../src/utils/csvImporter';
import type { AcademicCourse, AcademicCurriculum } from '../../src/utils/academicProgress';
import type { ClassScheduleRow, CsvParseIssue, ApplicableCourseRow } from '../../src/utils/csvImporter';
...
export async function autoLoadDepartmentCSVs(departmentId: string, entranceYear?: number): Promise<AutoLoadDepartmentCSVResult> {
  ...
  // Official courses are always loaded, including cohorts that have legacy CSVs.
  const officialCourses = dept && entranceYear ? await loadOfficialCourseCandidates({ ...dept, entranceYear }) : [];
  if (dept?.sourceStatus === 'partial_no_department_curriculum_pdf') {
    messages.push({ level: 'warning', text: '既存の計算用CSVには共通分野と時間割由来の補助科目を含みます。「履修資料」に取得済みの学科別PDFを収録しています。卒業判定前に条件を確認してください。' });
  }
  try {
    ...
    const curriculumCourses = parseCourses(timetableRows, departmentId);
    const mergedResult = scheduleCourses.length > 0
      ? mergeCoursesWithScheduleDetailed(curriculumCourses, scheduleCourses)
      : { courses: curriculumCourses, stats: { matchedByLectureCode: 0, matchedByCourseId: 0, matchedByTitle: 0, ambiguousMatches: 0, unmatchedOfferings: 0, unmatchedCourseRows: curriculumCourses.filter((course) => (course.offerings?.length ?? 0) === 0).length, unknownCourseTypes: curriculumCourses.filter((course) => course.courseType === 'unknown').length } };
    const normalizedTitle = (title: string) => title.normalize('NFKC').replace(/\s+/g, '').toLowerCase();
    const existingTitles = new Set(mergedResult.courses.map((course) => normalizedTitle(course.title)));
    const courses = [...mergedResult.courses.map((course) => ({ ...course, curriculumYear: entranceYear })),
      ...officialCourses.filter((course) => !existingTitles.has(normalizedTitle(course.title)))];
    ...
    if (mergedResult.stats.unknownCourseTypes > 0) {
      messages.push({ level: 'warning', text: `区分未確認の科目が${mergedResult.stats.unknownCourseTypes}件あります。` });
    }
    if (mergedResult.stats.unmatchedOfferings > 0) {
      messages.push({ level: 'warning', text: `科目マスタと結び付かなかった開講情報が${mergedResult.stats.unmatchedOfferings}件あります。` });
    }
    ...
    if (dept && entranceYear && entranceYear >= 2022 && entranceYear <= 2026) {
      try {
        const courses = officialCourses;
        if (courses.length > 0) {
          return {
            status: 'partial', referenceOnly: true, departmentId, departmentName, entranceYear,
            curriculum: { name: `${departmentName} ${entranceYear}年度（公式資料参照）`, requiredCredits: 0,
              breakdown: { required: 0, electiveRequired: 0, elective: 0 } },
            courses, applicableCourses: [], resources,
            stats: { requirementRows: 0, timetableRows: 0, scheduleRows: 0, curriculumCourses: courses.length,
              scheduleCourses: 0, mergedCourses: courses.length, coursesWithOfferings: 0, offerings: 0,
              matchedByLectureCode: 0, matchedByCourseId: 0, matchedByTitle: 0, ambiguousMatches: 0,
              unmatchedOfferings: 0, unmatchedCourseRows: courses.length, unknownCourseTypes: courses.length },
            messages: [{ level: 'warning', text: '当該入学年度の計算用CSVが未整備のため、公式PDFから抽出した参考科目を読み込みました。学部冊子には他学科科目も含まれます。必選・算入条件は「履修資料」で確認してください。卒業判定は未判定です。' },
              { level: 'info', text: `計算用CSV: ${error instanceof Error ? error.message : String(error)}` }],
          };
        }
      } catch (referenceError) {
        messages.push({ level: 'error', text: `公式資料の参考科目も読み込めませんでした: ${referenceError instanceof Error ? referenceError.message : String(referenceError)}` });
      }
    }
