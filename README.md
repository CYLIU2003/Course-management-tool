...
開発中：原本の区分階層と成績集計を修正しています。全資料の確認と本番更新は未完了です。[確認範囲・未解決事項・自己レビュー](docs/development_note_classification_ui.md)。

v0.3.0 本番反映（2026-09-08）：学部の学科・入学年度・コース別111組、731の必修・選択必修群と、大学院87組の修了最低単位をSQLiteからSupabaseへ反映しました。履修ガイドには対象科目、必選区分、修得済み・必要・残り単位を表示し、所属コースも保存します。学部19,338行の区分対応を検査済みです。本番参照データ373件と配信ファイルの一致を確認しました。原本内の数値不一致3件と、算入上限・個別許可等の全条件の自動判定は別途残っています。[原本照合・検証・公開状況](docs/development_note_degree_rules.md)。
...
履修条件の追加照合では、学部111コース・入学年度の731件の必修／選択必修群を登録し、群別進捗を実装しています。原本との科目対応・大学院条件の照合は継続中です。詳細は [開発記録](docs/development_note_degree_rules.md) を参照してください。
