...
v0.3.0 公開準備中：学部の学科・入学年度・コース別111組、731の必修・選択必修群と、大学院87組の修了最低単位をSQLiteに保存しました。履修ガイドには対象科目、必選区分、修得済み・必要・残り単位を表示し、所属コースも保存します。学部19,338行の区分対応を検査済みです。原本内の数値不一致3件と、算入上限・個別許可等の全条件の自動判定は別途残っています。本番更新は未完了です。[原本照合・検証・公開状況](docs/development_note_degree_rules.md)。
