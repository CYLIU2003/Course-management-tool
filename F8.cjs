...
txt = txt.replace(/export default function RequirementCategoryDetailDrawer[\s\S]*?const handleRetry = \(\) => \{\n    setReloadVersion\(\(v\) => v \+ 1\);\n  \};\n/, \xport default function RequirementCategoryDetailDrawer({ categoryId, onClose, curriculum, courses, allYearsData, applicableCourses }: RequirementCategoryDetailDrawerProps) {
