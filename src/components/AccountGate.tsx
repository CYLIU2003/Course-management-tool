...
import { accountRequest, responseError, setAccountToken, type Account, type StudentState } from '../api/account';
...
export default function AccountGate() {
  ...
  const loadAccount = useCallback(async () => {
    ...
    try {
      ...
      if (response.status === 401) { setAccount(null); setAccountToken(''); return; }
      if (!response.ok) throw new Error(await responseError(response));
      const value: Account = await response.json();
      setAccountToken(value.csrfToken); setAccount(value);
  ...
  return account ? <AccountSession key={account.id} account={account} onLogout={() => { setAccountToken(''); setAccount(null); }} /> : <SignIn onAuthenticated={loadAccount} />;
}
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await accountRequest(`/api/auth/${registering ? 'register' : 'login'}`, 'POST', {
        username: form.get('username'), password, ...(registering ? { departmentId: department, entranceYear: Number(year) } : {}),
      });
      if (!response.ok) throw new Error(await responseError(response));
  ...
  return <main className="account-page"><div className="account-intro"><span className="app-brand__mark" aria-hidden="true">c.</span><p className="eyebrow">CAMPUS NOTE · 仮公開版</p><h1>大学生活の履修を、<br />ひとつのノートに。</h1><p>時間割も、成績も、あなたのアカウントに保存。<br />学科・入学年度に合った科目から始められます。</p><p className="account-note">東京都市大学の学生向け非公式ツールです。</p></div>
    <section className="account-card"><div className="account-tabs"><button type="button" aria-pressed={!registering} disabled={busy} onClick={() => { setRegistering(false); setMessage(''); }}>ログイン</button><button type="button" aria-pressed={registering} disabled={busy} onClick={() => { setRegistering(true); setMessage(''); }}>新規登録</button></div>
      ...
      <form onSubmit={submit}><fieldset disabled={busy}>
        <label>ユーザー名<input name="username" required minLength={3} maxLength={32} pattern="[A-Za-z0-9_\-]+" autoComplete="username" autoCapitalize="none" spellCheck={false} aria-describedby="username-hint" /></label><small id="username-hint">半角英数字・_・- の3〜32文字（大文字・小文字は区別しません）</small>
        ...
        {registering && <><small>12文字以上。大学のアカウントとは別のパスワードを設定してください。</small><label>パスワード（確認）<input name="confirm" type="password" required minLength={12} maxLength={128} autoComplete="new-password" /></label>
          ...
          <p className="account-note">登録情報と入力した時間割・成績をサーバーに保存します。現在、メールによるパスワード再発行はありません。パスワードは大切に保管してください。</p></>}
...
function AccountSession({ account, onLogout }: { account: Account; onLogout: () => void }) {
  ...
  const latest = useRef<string | null>(account.state ? JSON.stringify(account.state) : null);
  const saved = useRef(latest.current);
  const running = useRef(false);
  const blocked = useRef(false);
  const [pending, setPending] = useState(false);
  const [message, setMessage] = useState('保存済み');
  ...
  const persist = useCallback(async () => {
    ...
    try {
      ...
      setPending(false); setError(false); setMessage('保存済み');
    } catch (reason) {
  ...
  const onStateChange = useCallback((state: StudentState) => {
    ...
    latest.current = next; setPending(true); void persist();
  }, [persist]);
  useEffect(() => {
  ...
  return <><aside className="account-bar print:hidden"><strong>{account.username}</strong><span role={error ? 'alert' : 'status'}>{message}</span>{error && <>{!conflict && <button onClick={() => { blocked.current = false; void persist(); }}>保存を再試行</button>}<button onClick={exportUnsaved}>入力内容をダウンロード</button><button onClick={() => window.location.reload()}>再読み込み</button></>}<button disabled={pending || loggingOut} onClick={logout}>{loggingOut ? 'ログアウト中…' : 'ログアウト'}</button></aside><TimetableApp account={account} onStateChange={onStateChange} /></>;
}
