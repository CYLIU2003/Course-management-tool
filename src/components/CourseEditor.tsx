...
export default function CourseEditor({ initial, day, periodId, onSave, onClear, onClose, courses, canDelete }: {
  ...
  return <dialog ref={dialog} className="course-editor" onCancel={onClose} aria-labelledby="editor-title">
    <form onSubmit={event => { event.preventDefault(); if (draft.title.trim() && validCredits) onSave({ ...draft, title: draft.title.trim(), credits: credits === '' ? undefined : Number(credits) }); }}>
      ...
      <div className="course-editor__body">
        <label>科目名 <span className="small">必須</span><input autoFocus value={draft.title} onChange={e => setDraft({ ...draft, title: e.target.value, courseId: undefined, courseType: 'unknown' })} required placeholder="科目名を入力、または一覧から選ぶ" /></label>
        <button type="button" className="text-action" onClick={() => setSearchOpen(!searchOpen)}>{searchOpen ? '科目一覧を閉じる' : '科目一覧から選び直す'}</button>
        {searchOpen && <CourseSearchPanel courses={courses} onAdd={course => { setDraft({ ...draft, title: course.title, courseId: course.id, courseType: course.courseType }); setCredits(String(course.credits)); setSearchOpen(false); }} />}
