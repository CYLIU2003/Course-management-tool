...
export default function GuideProgress({ documents, department, data }: { documents: HandbookDocument[]; department: string; data: AcademicAllYearsData }) {
  ...
  return <section className="guide-progress" aria-label="区分ごとの科目と履修状況">
    ...
    <div className="guide-category-grid">{categories.filter(category => (!query && status === 'すべて') || matches.some(entry => entry.category === category)).map(category => {
      ...
      const minimum = guideMinimum(documents, category, department);
      return <article className="guide-category" key={category}>
        ...
        <details className="guide-course-disclosure" open={query !== '' || status !== 'すべて' ? true : undefined}><summary><span>該当科目を見る <b>{visible.length}</b></span><small>表示科目の合計 {sumCourseCredits(visible)}単位</small></summary>
          {[...new Set(visible.map(entry => entry.group))].map(group => <section className="guide-subgroup" key={group}><h5>{group}</h5><p>{visible.filter(entry => entry.group === group).length}科目 · 修得 {sumCourseCredits(visible.filter(entry => entry.group === group), '修得済み')}単位 · 予定 {sumCourseCredits(visible.filter(entry => entry.group === group), '履修予定')}単位</p><ul className="guide-course-list">{visible.filter(entry => entry.group === group).map(entry => <li key={entry.course.id}><div className="guide-course-name"><strong>{entry.title}</strong><small>{entry.printedRequirement ? `原本の必選記号：${entry.printedRequirement}` : '必修・選択の条件は原本確認'}</small><a href={localHandbookUrl(entry.document.localPath, entry.course.page)} target="_blank" rel="noreferrer">科目の出典 · PDF {entry.course.page}ページ ↗</a></div><div className="guide-course-result"><strong>{entry.course.credits}<small>単位</small></strong><span className="guide-course-status" data-status={entry.status}>{entry.status}</span></div></li>)}</ul></section>)}
        </details>
      </article>;
    })}</div>
  </section>;
