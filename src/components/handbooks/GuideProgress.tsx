    ...
    <div className="guide-category-grid">{categories.filter(category => (!query && status === 'すべて') || matches.some(entry => entry.category === category)).map(category => {
      ...
      return <article className="guide-category" key={category}>
        ...
        <details className="guide-course-disclosure" open={query !== '' || status !== 'すべて' ? true : undefined}><summary><span>該当科目を見る <b>{visible.length}</b></span><small>表示科目の合計 {sumCourseCredits(visible)}単位</small></summary>
          {[...new Set(visible.map(entry => entry.group))].map(group => <section className="guide-subgroup" key={group}><h5>{group}</h5><p>{visible.filter(entry => entry.group === group).length}科目 · 修得 {sumCourseCredits(visible.filter(entry => entry.group === group), '修得済み')}単位 · 予定 {sumCourseCredits(visible.filter(entry => entry.group === group), '履修予定')}単位</p><ul className="guide-course-list">{visible.filter(entry => entry.group === group).map(entry => <li key={entry.course.id}><div className="guide-course-name"><strong>{entry.title}</strong><small>{entry.printedRequirement ? `原本の必選記号：${entry.printedRequirement}` : '必修・選択の条件は原本確認'}</small></div><div className="guide-course-result"><strong>{entry.course.credits}<small>単位</small></strong><span className="guide-course-status" data-status={entry.status}>{entry.status}</span></div></li>)}</ul></section>)}
        </details>
      </article>;
    })}</div>
  </section>;
