...
export async function loadDepartmentCurriculum(departmentId: string, entranceYear: number, signal?: AbortSignal): Promise<CurriculumDataset> {
  const response = await apiFetch(`/api/curricula/${encodeURIComponent(departmentId)}/${entranceYear}`, { signal });
